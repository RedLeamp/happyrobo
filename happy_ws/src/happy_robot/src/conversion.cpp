...
class LaserScanCombiner {
  ...
  public:
    explicit LaserScanCombiner(ros::NodeHandle& n) : nh_(n) {
      // Topic names.
      const std::string kScan1Topic = "/front/scan";
      const std::string kScan2Topic = "/nanosystems_points_01";
      const std::string kCombinedScanTopic = "/conversion";
  ...
  protected:
    ...
    bool setTFScan1To2() {
      ...
      if ( kAssumeStaticTransform ) {
        ...
        if ( !tf_is_known_ ) {
        ROS_INFO_STREAM("Waiting for transform between scan 1 and scan 2 frames: " <<
          ...
          try {
            ...
            ROS_INFO_STREAM("Error while looking up transform between scan 1 and scan 2 : " <<
              ex.what());
            return true;
          }
          tf_is_known_ = true;
        ...
        ROS_INFO_STREAM("Dynamic transforms are not implemented.");
        ros::shutdown;
      }
      return false;
    }
    ...
    void scan2Callback(const sensor_msgs::PointCloud2::ConstPtr& msg) {
      ...
      if ( !reference_scan2_is_set_ ) {
        ...
        ROS_INFO_STREAM("First scan set as reference scan for sensor 2.");
        return;
      }
      ...
      try {
        for ( size_t i = 0; i < scan2_in_frame1_xyzi.channels.size(); i++ ) {
          if ( scan2_in_frame1_xyzi.channels.at(i).name == "intensity" ) {
            ...
            ROS_INFO_STREAM("In scan 2 cloud, mismatch between intensity and points array sizes.");
          }
        }
      } catch (const std::exception& e) {
        ROS_INFO_STREAM("Could not find intensities for scan 2 cloud: " << e.what());
      }
      ...
      for ( size_t i = 0; i < scan2_in_frame1_xyzi.points.size();  i++ ) {
        ...
        ROS_INFO_STREAM("min angle: " << combined_scan.angle_min);
        ROS_INFO_STREAM("max angle: " << combined_scan.angle_max);
        ROS_INFO_STREAM("angle inc: " << combined_scan.angle_increment);
        ROS_INFO_STREAM("rel angle: " << relative_angle);
